{
  description = "Flake for the StoreFront service";
  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        gradleDeps = pkgs.runCommand "storefront-gradle-deps" {
          src = ./.;
          buildInputs = [ pkgs.gradle pkgs.jdk21 ];
          outputHashAlgo = "sha256";
          outputHashMode = "recursive";
          outputHash = "sha256-qK6025GUn6y5Yn/JnyXkUSfWIeznATQmulC1NJhV0Q0=";
        } ''
          cp -r $src/* ./
          chmod +x ./gradlew
        ...
        gradleBuild = pkgs.stdenv.mkDerivation {
          ...
          installPhase = ''
            mkdir -p $out/libs
            cp build/libs/storefront-0.0.1-SNAPSHOT.jar $out/libs/app.jar
          '';
        };
